...
TEST_F(ConfigParserTest, ParseHost) {
    // Valid cases
    EXPECT_NO_THROW(parser.parseGlobal("host 127.0.0.1", config));
    EXPECT_EQ(config.host, "127.0.0.1");

    // Invalid cases
    EXPECT_THROW(parser.parseGlobal("host invalid_host", config), ConfigError);
    EXPECT_THROW(parser.parseGlobal("host 1270.0.5.1", config), ConfigError);
    EXPECT_THROW(parser.parseGlobal("host ", config), ConfigError);

    // Duplicate host
    EXPECT_THROW(parser.parseGlobal("host 192.168.1.1", config), ConfigError);
}

TEST_F(ConfigParserTest, ParsePort) {
    // Valid cases
    EXPECT_NO_THROW(parser.parseGlobal("port 8080", config));
    EXPECT_EQ(config.port, 8080);

    // Invalid cases
    EXPECT_THROW(parser.parseGlobal("host invalid_port", config), ConfigError);
    EXPECT_THROW(parser.parseGlobal("port ", config), ConfigError);
    EXPECT_THROW(parser.parseGlobal("port 8081578", config), ConfigError);

    // Duplicate host
    EXPECT_THROW(parser.parseGlobal("port 8081", config), ConfigError);
}

// ServerName is optional so it can be empty
TEST_F(ConfigParserTest, ParseServerName) {
    // Valid cases
    EXPECT_NO_THROW(parser.parseGlobal("server_name localhost", config));
    EXPECT_NO_THROW(parser.parseGlobal("", config));

    // Duplicate server name
    EXPECT_THROW(parser.parseGlobal("server_name localhost", config), ConfigError);
}

TEST_F(ConfigParserTest, ParseErrorPage) {
    // Valid cases
    EXPECT_NO_THROW(parser.parseGlobal("error_page 404 config/default/404.html", config));

    // Invalid cases
    EXPECT_THROW(parser.parseGlobal("error_page 404 config/default//404.html", config), ConfigError);
    EXPECT_THROW(parser.parseGlobal("error_page 404 config/default/505.html", config), ConfigError);
